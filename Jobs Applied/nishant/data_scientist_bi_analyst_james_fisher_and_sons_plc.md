# Data Scientist / BI Analyst

- **Company:** James Fisher and Sons plc
- **Location:** Barrow-In-Furness, England, United Kingdom
- **Applied by:** Nishant
- **Applied date:** 2026-08-01T17:32:22.705Z
- **Licensed sponsor:** James Fisher and Sons plc
- **Original posting:** https://uk.linkedin.com/jobs/view/data-scientist-bi-analyst-at-james-fisher-and-sons-plc-4444198423?position=3&pageNum=5&refId=hVStYJRkryB71IMXJSIuiA%3D%3D&trackingId=BU3IaSob3fEMr%2F36W9kZKw%3D%3D
- **Application portal:** https://uk.linkedin.com/jobs/view/data-scientist-bi-analyst-at-james-fisher-and-sons-plc-4444198423?position=3&pageNum=5&refId=hVStYJRkryB71IMXJSIuiA%3D%3D&trackingId=BU3IaSob3fEMr%2F36W9kZKw%3D%3D
- **Resume used:** Nishant Sharma
- **Resume originally designed for:** https://jamesfisherweb.eploy.net/vacancies/1878/data-scientist--bi-analyst.html?utm_source=adzuna&utm_medium=adzuna&cmpid=1000-162

## Job description captured at application time

Data Scientist/BI AnalystPermanent position

UK - Home based

The Role

The Data Scientist / Power BI Analyst will play a key role in transforming organisational data into actionable insight, developing business intelligence solutions, and supporting key enterprise systems including IFS.

Key Duties And Responsibilities

Data Analytics & Modelling:

- Develop high-quality data models using Power BI Desktop and Power Query.
- Build interactive dashboards and reports informed by best practices such as semantic modelling and star schema design.
- Perform statistical analysis, data mining, and predictive modelling.

Power BI Platform Ownership

- Create, maintain, and optimise Power BI datasets, dataflows, and scheduled refreshes.
- Manage secure access, workspace permissions, and sharing practices.

Business Systems Reporting & Support

- Work with data structures including views, tables, and custom fields.
- Use debugging, view extraction and data modelling workflows to source and model operational data.
- Provide second-line functional support for IFS as required.

System Support & Integration

- Provide general system support across enterprise applications.
- Assist with configuration, system documentation, user training, and adoption sessions.

Cross-Functional Collaboration

- Work with finance, supply chain, operations and commercial teams to translate business requirements into technical solutions.
- Support change management and continuous improvement initiatives.

What We’re Looking For

Strong analytical and problem-solving skills and the ability to analyse complex data requirements and design effective solutions.

Detail-oriented with meticulous attention to detail to ensure data accuracy and integrity.

A creative and innovative thinker who can develop new solutions to meet evolving data challenges.

A strong team player with the ability to collaborate effectively with cross-functional teams.

Be adaptable and resilient with the ability to adapt to changing business needs and technology advancements.

Strategic mindset with the ability to align data architecture with the organisation's long-term goals.

Effective communication with all stakeholders and Directors.

Key Skills, Knowledge And Experience

- Degree in Data Science, Data Analysis, Computer Science, Information Systems, Mathematics, or related field.
- Microsoft Power BI certifications are desirable.
- Strong experience in Power BI, including Power Query, DAX, semantic modelling, and dataflows.
- Good working knowledge of data warehousing, star schema, and relational databases.
- Experience working with IFS including views, tables, and debugging tools.
- SQL skills and understanding of ETL processes.
- Strong Excel capabilities.

Why join us?

Competitive benefits tailored to the division.

Opportunities for career growth and visibility across the organisation.

A supportive, inclusive culture that values your ideas and contributions.

Work that truly makes a difference in safety, innovation, and global capability.

About Us

James Fisher is a global engineering services company with over 50 years of experience delivering complex offshore energy projects in some of the world’s most challenging environments. We operate across Energy, Defence, and Maritime Transport, leveraging cutting-edge technology and deep expertise to support the full lifecycle of our customers’ assets.

In the Energy Division, we provide safe, efficient solutions across oil & gas and renewables - from well support and full-field decommissioning to integrated services for offshore wind and expert subsea inspection, repair, and maintenance. Our work helps operators extend asset life, reduce downtime, and respond rapidly to operational challenges, driving the global shift toward a cleaner, more sustainable future.

Visit our website at: www.james-fisher.com

JFD is the world leading underwater capability provider, serving the commercial and defence markets with innovative diving, submarine and hyperbaric rescue, technical solutions and services.

Operating worldwide, the company is the world’s leading provider of submarine rescue capability and an established provider of submarine escape training. The company is at the forefront of Hyperbaric Rescue, along with being the leading supplier of commercial and defence diving equipment and saturation diving systems to the commercial industry.

www.jfdglobal.com

Maritime Transport - With nearly two centuries of maritime shipping and innovation, James Fisher has an unrivalled reputation for quality, safety and reliability.

Our long-standing customer partnerships allow us to deliver the highest possible standard of maritime solutions. As a global specialist in ship-to-ship transfer and coastal shipping, James Fisher is a recognised market leader in the movement of the world’s critical resources.

Ensuring safety at sea and in port, and investing in the fleet of tomorrow, James Fisher is shaping the future of maritime.

If this sounds like the ideal next step in your career, then click ‘apply’ now!

Due to the volume of applications, we receive for our vacancies, on occasion applications may close before the deadline, so please apply early to avoid disappointment.

James Fisher and Sons are committed to taking positive action on diversity and strongly encourage applications for candidates from all backgrounds. We are proud to be a Disability Confident employer and recognise that our success depends on our talented and diverse workforce.

About You

About Us

Not Specified
